...
public class FactionsUUIDHook extends FactionsHook implements Listener {
	...
	public void onClaim(LandClaimEvent e) {
		...
		Bukkit.getScheduler().runTask(tempfly, () -> {
			...
			if (!claimer.equals(current)) {
				super.onLandOverClaimed(loc.getChunk(), getFactionWrapper(current),
					getFactionWrapper(claimer));
			} else {
	...
	public void onRelation(FactionRelationEvent e) {
		super.onFactionRelationshipChange(getFactionWrapper(e.getFaction()),
			getFactionWrapper(e.getTargetFaction()));
	}
	...
	public void onFactionJoin(FPlayerJoinEvent e) {
		Bukkit.getScheduler().runTask(tempfly, () -> {
			if (e.getfPlayer().getPlayer().isOnline()) {
				super.onPlayerJoinFaction(e.getfPlayer().getPlayer(),
					getFactionWrapper(e.getFaction()));	
			}
		});
	}
	...
	public void onFactionLeave(FPlayerLeaveEvent e) {
		Bukkit.getScheduler().runTask(tempfly, () -> {
			if (e.getfPlayer().getPlayer().isOnline()) {
				super.onPlayerLeaveFaction(e.getfPlayer().getPlayer(),
					getFactionWrapper(e.getFaction()));	
			}
		});
	}
	...
	public UUID getFactionOwner(FactionWrapper faction) {
		return UUID.fromString(((Faction)
			faction.getRawTerritory()).getFPlayerAdmin().getId());
	}
	...
	public boolean isEnemy(UUID playerId, FactionWrapper faction) {
		return getFPlayer(playerId).getFaction().getRelationTo((Faction)
			faction.getRawTerritory()).isEnemy();
	}

	@Override
	public boolean isAllied(UUID playerId, FactionWrapper faction) {
		return getFPlayer(playerId).getFaction().getRelationTo((Faction)
			faction.getRawTerritory()).isAlly();
	}
	...
	public FactionWrapper getFactionAt(Location loc) {
		return getFactionWrapper(Board.getInstance().getFactionAt(new
			FLocation(loc)));
	}
